...
app.post('/available-friend-lobbies', async (req, res) => {
  const { myId } = req.body;
  ...
  try {
    const lobbies = await client.query(`
      SELECT l.*
      FROM lobby l
      JOIN friends f ON f.friend_id = l.host_id
      WHERE f.user_id = $1 AND l.status = 'waiting' AND l.guest_id IS NULL
    `, [myId]);

    res.json(lobbies.rows);
  } catch (err) {
    console.error('Error al obtener lobbys:', err);
